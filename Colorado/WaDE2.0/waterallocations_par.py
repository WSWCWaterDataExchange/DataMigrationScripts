...
workingDir="C:/tseg/testData/100/"
os.chdir(workingDir)
...
# slide df to all
numrows_per_proc = len(df100.index) / size
extra_cells = len(df100.index) % size
if rank == 0:
    lower_bound = 0
    upper_bound = numrows_per_proc + extra_cells  # add extras to root
else:
    lower_bound = rank * numrows_per_proc + extra_cells
    upper_bound = (1+rank)*numrows_per_proc + extra_cells
"""
if rank == 0:
    df100 = df100.iloc[0: numrows_per_proc + extra_cells]  # add extras to root overlap cells at boundary
else:
    df100 = df100.iloc[rank * numrows_per_proc + extra_cells: (1+rank)*numrows_per_proc + extra_cells]  # overlap cells at boundary
"""

# divide data frame among procs.
df100 = df100.iloc[lower_bound - 1 : upper_bound + 1]  # overlap cells at boundary
